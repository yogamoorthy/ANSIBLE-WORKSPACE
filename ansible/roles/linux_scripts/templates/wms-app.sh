...
#------ Source the ENV Vars -------------------------------------------------------------------------------------------#
. /opt/hy-vee/scripts/env-setup.sh
...
gather_info () {
  # Gather process IDs
  mip_process=$(ps -ef | grep java | grep $mip_name/profile-root | awk '{print $2}')
  mda_process=$(ps -ef | grep java | grep $mda_name/profile-root | awk '{print $2}')
  wm_process=$(ps -ef | grep java | grep $wm_name/profile-root | awk '{print $2}')
  lm_process=$(ps -ef | grep java | grep $lm_name/profile-root | awk '{print $2}')
  db_process=$(ps -ef | grep java | grep $db_name/profile-root | awk '{print $2}')
  # Gather uptime stats
