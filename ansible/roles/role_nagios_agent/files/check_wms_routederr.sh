...
file=`find /apps/scope/mif/transfer/$ENVIRONMENT/routedorders/error/ -maxdepth 1 -type f |wc -l`


if [ "$file" -ge "$2" ]; then
  echo "SalesOrder failed transfer - $ENVIRONMENT | Errors = $file"
  exit 2
else
  echo "SalesOrder transfer OK - $ENVIRONMENT | Errors = $file"
  exit 0
fi
