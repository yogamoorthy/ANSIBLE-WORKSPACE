...
if [ "$1" = "-w" ] && [ "$2" -gt "0" ] && [ "$3" = "-e" ]; then
  ...
  query() {
    ...
    from (
    select ref_shipment_nbr as "Route",
    pickup_start_dttm as "Depart Time",
    d_facility_alias_id as "Store",
    ref_stop_seq as "Stop",
    count(distinct d_facility_alias_id) over (partition by ref_shipment_nbr, ref_stop_seq) as "Error"
    from $wm_db_schemaname.orders
    where do_status = 110
    and ref_shipment_nbr is not null)
    where "Error" > 1;
ENDOFSQL
  }
    query_return=$(query | tail -n +$((4 + 1)) | wc -l)

    if [ "${query_return}" -ge "$2" ]; then
        echo "Overlapping routes has $query_return errors - $wm_env | Errors = $query_return"
        exit 1
    else
        echo "Overlapping routes has $query_return errors - $wm_env | Errors = $query_return"
        exit 0
