...
query() {
    sqlplus -S $wm_db_username/$secret@$wm_db_servicename <<ENDOFSQL
        ...
        AND (LAST_UPDATED_DTTM BETWEEN sysdate - INTERVAL '7' DAY AND sysdate - INTERVAL '10' MINUTE);
        EXIT;
ENDOFSQL
}
    ...
    if [ "$query_return" -ge "$4" ]; then
        echo "Integration - ShipmentConfirm has $query_return errors - $wm_env | Errors = $query_return"
        exit 2
