...
if [ "$1" = "-w" ] && [ "$2" -gt "0" ] && [ "$3" = "-c" ] && [ "$4" -gt "0" ] && [ "$5" = "-e" ]; then
  ...
  query() {
    sqlplus -S $wm_db_username/$secret@$wm_db_servicename <<ENDOFSQL
        select rule_id, tbl_name, colm_name, oper_code, rule_cmpar_value, mod_date_time, user_id, last_updated_dttm
        from $wm_db_schemaname.rule_sel_dtl where rule_id in ('1009296','2695682','2768435','3541481','2526022', '1004184')
...
ENDOFSQL
    ...
    if [ "$query_return" -ge "$4" ]; then
        echo "There are $query_return chasewaves - $wm_env | Errors = $query_return"
        exit 2
