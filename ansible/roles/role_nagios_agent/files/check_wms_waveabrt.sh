...
query() {
    sqlplus -S $wm_db_username/$secret@$wm_db_servicename <<ENDOFSQL
      SELECT * FROM $wm_db_schemaname.SHIP_WAVE_PARM WHERE STAT_CODE =  89
      and CREATE_DATE_TIME >= sysdate -3;
ENDOFSQL
}
    ...
    if [ "$query_return" != "0" ]; then
        echo "WMS Team - Aborted Waves has $query_return errors - $wm_env | Errors = $query_return"
       exit 2
    else
       echo "WMS Team - Aborted Waves has $query_return errors - $wm_env | Errors = $query_return"
       exit 0
