...
where name = 'VoCollectEndpoint_PROD';
ENDOFSQL
    }
    ...
    if [ "$query_return" -lt "$2" ]; then
        echo "Vocollect MHE Endpoints are down in - $wm_env"
        exit 2
    else
        echo "Vocollect MHE Endpoints are up in - $wm_env"
        exit 0
    fi
    ...
    echo "# EXAMPLE:  /usr/lib64/nagios/plugins/check_wms_vocollectmhe_endpoints.sh -c 2 -e"
    exit
fi
