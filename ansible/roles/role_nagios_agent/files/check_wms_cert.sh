...
function checkExpiration {
  ...
  if [[ $days_left -le 14 ]]; then
    msg="CRITICAL: SSL certificate expires in $days_left days! - $host"
    echo -e "$msg"
    exit 2
    ...
    msg="WARNING: SSL certificate expires in $days_left days. - $host"
    echo -e "$msg"
    exit 1
  else
    msg="SSL certificate expires in $days_left days. - $host"
    echo -e "$msg"
    exit 0
  fi
}
