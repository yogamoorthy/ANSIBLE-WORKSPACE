...
if [ "$1" = "-c" ] && [ "$2" -gt "0" ] && [ "$3" = "-e" ]; then
 ...
 query() {
    sqlplus -S $wm_db_username/$secret@$wm_db_servicename <<ENDOFSQL
        select *
        from $wm_db_schemaname.LISTENER
        where enable = '0';
ENDOFSQL
 }
    ...
    if [ "$query_return" -ge "$2"]; then
        echo "There are $query_return listeners off - $wm_env | Errors = $query_return"
        exit 2
    else
        echo "There are $query_return listeners off - $wm_env | Errors = $query_return"
        exit 0
    ...
    echo "# EXAMPLE:  /usr/lib64/nagios/plugins/check_listeners -c 1 -e"
    exit
fi
