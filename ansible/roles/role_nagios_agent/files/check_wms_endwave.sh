...
query() {
    ...
    and trunc(mod_date_time) = trunc(sysdate);
ENDOFSQL
}
    ...
    if [ "$query_return" -ge "$2" ]; then
        echo "End Wave has $query_return errors - $wm_env | Errors = $query_return"
        exit 1
    else
        echo "End Wave has $query_return errors - $wm_env | Errors = $query_return"
        exit 0
    ...
    echo "# EXAMPLE:  /usr/lib64/nagios/plugins/check_wms_endwave -w 1 -e (XI HOSTNAME VARIABLE HERE)"
    exit
fi
