...
if [ "$1" = "-c" ] && [ "$2" -gt "0" ] && [ "$3" = "-e" ]; then
    ...
    query() {
        ...
        select wave_nbr,tc_lpn_id from $wm_db_schemaname.lpn 
        where container_type ='VAS'
        and lpn_facility_status <'20'
        and wave_nbr in(select ship_wave_nbr from $wm_db_schemaname.ship_wave_parm
        where stat_code ='90');
ENDOFSQL
    }
    ...
    if [ "$query_return" -ge "$2" ]; then
        echo "VAS LPNs has $query_return errors - $wm_env | Errors = $query_return"
        exit 2
    else
        echo "VAS LPNs has $query_return errors - $wm_env | Errors = $query_return"
        exit 0
