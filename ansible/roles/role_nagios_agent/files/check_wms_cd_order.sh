...
if [ "$1" = "-c" ] && [ "$2" -gt "0" ] && [ "$3" = "-e" ]; then
    ...
    SELECT
        hv_crossdock.cd_parentorder
    FROM
        $wm_db_schemanamehv_crossdock
    WHERE ((CD_PROSPEROSENT = 0 and CD_WMSSENT = 0)
    and CD_UPDATE_DTTM = sysdate - INTERVAL '1' HOUR
    OR CD_FACILITYALIASID is null OR CD_STOP is null)
    GROUP BY
        hv_crossdock.cd_parentorder
    HAVING
        COUNT(hv_crossdock.cd_parentorder) > '1';
ENDOFSQL
    }
    ...
    if [ "$query_return" -ge "$2" ]; then
        echo "Integration -  2 orders have the same parentorder $wm_env"
        exit 2
    else
        echo "Integration - No parent order errors - $wm_env | Errors = $query_return"
        exit 0
    fi
else
