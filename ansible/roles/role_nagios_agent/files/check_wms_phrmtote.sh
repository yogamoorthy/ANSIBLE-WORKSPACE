...
if [ $6 = "chrdevapp1-v-hbc" ]; then
  source /opt/hy-vee/scripts/hbc/env-setup.sh
  source /opt/hy-vee/scripts/env-setup.sh
elif [ $6 = "chrapp1a-v-hbc" ]; then
  source /opt/hy-vee/scripts/hbc/env-setup.sh
else
  echo -e "# Usage:\t$sName -w -c -e "
  echo -e "\t\t= invalid hostname"
  echo "# EXAMPLE:  /usr/lib64/nagios/plugins/check_tran -w 1 -c 2 -e (XI HOSTNAME VARIABLE HERE)"
  exit
fi
...
WHERE event_id      IN (9400,9401,9402,9403,9500,9501,9600,9601,9602,9603)
AND (stat_code = 20)
...
if [ "$query_return" -ge "$4" ]; then
  echo "Integration - Pharmacy Tote has $query_return errors - $wm_env | Errors = $query_return"
  exit 2
